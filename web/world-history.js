...
export async function worldHistoryScreen(section,params){
 ...
 return heading('LE MARCHÉ MONDIAL','Historique des mouvements','Tous les clubs, actifs et dormants, saison par saison.')+nav+seasonNavigation(data)+partial+(type==='academy'?'<p class="muted">Informations à la promotion lorsqu’elles sont archivées ; sinon, données actuelles du joueur.</p>':'')+card(`${labels[type]} · ${data.total}`,academy??(table(headers,rows)+pager(data)));
}

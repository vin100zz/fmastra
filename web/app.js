...
import {api,escape as e,number as n,date,season,card,stat,heading,empty,toast,setNations} from './ui.js';
...
async function refreshState(){state=await api('/monde/etat');document.querySelector('#season-label').textContent=state.exists?`SAISON ${season(state.season)}`:'VOTRE UNIVERS FOOTBALL';document.querySelector('#game-date').textContent=state.exists&&!state.recovery_required?date(state.date,true):'Bienvenue sur le banc de touche';document.querySelector('#market-badge').textContent=state.market?'Mercato ouvert':'';if(state.exists&&!state.recovery_required){leagues=await api('/competitions');if(!nationsLoaded){setNations(await api('/nations'));nationsLoaded=true;}document.querySelector('#leagues-nav').innerHTML=leagues.map(league=>`<a href="#/league/${league.id}" data-nav="league-${league.id}"><span class="league-code">${e(league.nation.slice(0,2))}</span>${e(league.name)}</a>`).join('');}busyButtons();if(state.job)pollJob(state.job);}
...
async function render(){const version=++renderVersion;const {parts,params}=routeParts();if(!main.innerHTML||main.querySelector('.loading'))main.innerHTML='<div class="loading">Chargement…</div>';
 ...
 try{await refreshState();let html;if(!state.exists||state.recovery_required)html=await savesScreen(true);else{const [screen,id,section]=parts;switch(screen){case 'clubs':html=await clubsScreen(params);break;case 'club':html=await clubScreen(id,section,params);break;case 'league':html=await leagueScreen(id,section,params,leagues);break;case 'transfers':html=await worldHistoryScreen(id,params);break;case 'players':html=await playersScreen(params);break;case 'player':html=await playerScreen(id,section,params);break;case 'match':html=await matchScreen(id,section);break;case 'saves':html=await savesScreen();break;case 'journal':html=await journalScreen(params);break;default:html=await dashboard(leagues);}}

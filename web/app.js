...
main.addEventListener('submit',async event=>{event.preventDefault();const element=event.target;const data=new FormData(element);if(element.matches('[data-filter]')){const values=Object.fromEntries(data);Object.keys(values).forEach(key=>{if(!values[key])delete values[key];});changeParams(values);}else if(element.id==='new-game'){if(state.exists){const dialog=document.querySelector('#confirm-dialog');dialog.showModal();const confirmed=await new Promise(resolve=>dialog.addEventListener('close',()=>resolve(dialog.returnValue==='confirm'),{once:true}));if(!confirmed)return;}await command('/partie/creer',{graine:Number(data.get('seed'))});}else if(element.id==='save-game')await command('/partie/sauvegarder',{slot:data.get('slot')});});
main.addEventListener('click',event=>{const button=event.target.closest('button');if(!button)return;const {params}=routeParts();if(button.dataset.season){changeParams({saison:button.dataset.season});}if(button.dataset.page){params.set('page',button.dataset.page);changeParams(params);}if(button.dataset.sort){const current=params.get('tri')||(routeParts().parts[0]==='players'?'value':'rating');params.set('ordre',current===button.dataset.sort&&(params.get('ordre')||'desc')==='desc'?'asc':'desc');params.set('tri',button.dataset.sort);params.delete('page');changeParams(params);}if(button.dataset.command==='load')command('/partie/charger',{slot:button.dataset.slot});if(button.id==='retry')render();});

...
export async function playerScreen(id,section,params){
 ...
 if(section==='profile'){
  ...
  const totals=data.career.totals;
  ...
  content=card('Évolution du niveau',curve)+card('La carrière',table(['SAISON','CLUB','TRANSFERT','COMPÉTITION','MATCHS','BUTS','PASSES','NOTE'],data.career.items.map(row=>[season(row.season),clubLink(row.club),row.fee?money(row.fee):'—',e(row.competition||'Marché extérieur'),row.matches,row.goals,row.assists,row.average?n(row.average):'—']),footer));
 }

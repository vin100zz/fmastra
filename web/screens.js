...
export async function playersScreen(params){
 ...
 const filter=`<form data-filter><div class="filters"><input name="recherche" type="search" value="${value('recherche')}" placeholder="Rechercher un joueur…" aria-label="Rechercher un joueur">${select('poste','Tous les postes',['GB','DC','DL','DR','MDC','MC','MOC','AILG','AILD','BU'].map(role=>[role,role]))}${select('statut_club','Tous les clubs',[['actif','Clubs actifs'],['dormant','Clubs dormants']])}${select('contrat','Tous les contrats',[['libre','Agents libres'],['sous_contrat','Sous contrat']])}</div><details class="filters" ${['age_min','age_max','niveau_min','nation','club','salaire_min','salaire_max','valeur_max'].some(key=>params.get(key))?'open':''}><summary>Filtres avancés</summary><div class="filters"><label>Âge minimum <input name="age_min" type="number" min="0" max="100" value="${value('age_min')}"></label><label>Âge maximum <input name="age_max" type="number" min="0" max="100" value="${value('age_max')}"></label><label>Niveau minimum <input name="niveau_min" type="number" min="1" max="200" value="${value('niveau_min')}"></label><label>Nation (code) <input name="nation" value="${value('nation')}" placeholder="FRA"></label><label>Club (ID) <input name="club" type="number" value="${value('club')}"></label><label>Salaire min. (€/mois) <input name="salaire_min" type="number" min="0" value="${value('salaire_min')}"></label><label>Salaire max. (€/mois) <input name="salaire_max" type="number" min="0" value="${value('salaire_max')}"></label><label>Valeur max. (M€) <input name="valeur_max" type="number" min="0" step="any" value="${value('valeur_max')}"></label></div></details></form>`;
 return heading('Joueurs')+filter+card(`${n(data.total)} joueurs`,playerTable(data,true,params.get('tri')||'value',params.get('ordre')||'desc'));
}

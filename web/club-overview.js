...
// One line per match: opponent, plane if away, then the competition when it is not the club's championship.
function matchRow(match,club){
 ...
 const note=[match.competition!==club.competition?match.competition:'',shootout].filter(Boolean).join(' · ');
 const detail=`${match.competition} · ${match.round_label} · ${home?'Domicile':'Extérieur'}${shootout?` · ${shootout}`:''}`;
 return `<a class="club-match" href="#/match/${match.id}" title="${e(detail)}"><span class="club-match-date">${shortDate(match.date)}</span><span class="club-match-who"><b>${kitDot(opponent)}${e(opponent.name)}</b>${home?'':awayIcon}${note?`<small>${e(note)}</small>`:''}</span>${score}</a>`;
}
...
function calendarBlock(club,data){
 return card('Calendrier',`<div class="card-body">${matchList(data.last,club,'Aucun match joué.')}${matchList(data.next,club,'Aucun match programmé.')}</div>`,`<a href="#/club/${club.id}/calendar" aria-label="Voir le calendrier">Voir →</a>`);
}

function financeBlock(club,data){
...
function lineupBlock(club,lineup){

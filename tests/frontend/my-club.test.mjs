...
const news={items:[{id:1,date:'2029-08-02',kind:'offer_received',text:'Nice propose 3 M€.',player_id:11,match_id:null,read:false},{id:0,date:'2029-08-01',kind:'season',text:'Ouverture.',player_id:null,match_id:null,read:true}],total:2,page:1,page_size:30,unread:1};
...
test('the dashboard shows an inbox with unread entries and the club widgets, without tabs',async()=>{
 ...
 assert.match(html,/<a href="#\/player\/11" class="inbox-item unread" data-news="1">/);
 assert.match(html,/<button type="button" class="inbox-item" data-news="0">/);
